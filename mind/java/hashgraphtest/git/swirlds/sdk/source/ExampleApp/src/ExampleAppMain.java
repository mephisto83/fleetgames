...
public class ExampleAppMain implements SwirldMain {
	...
	public void init(Platform platform, int id) {
		...
		platform.setAbout("Hello Swirld from andrews machine v. 1.0\n"); // set the browsers "about" box
	...
	public void run() {
		...
		console.out.println("Hello Swirld from " + myName + " in Andrews machine.");
		...
		while (true) {
			...
			if (!lastReceived.equals(received)) {
				...
				console.out.println("Received it : " + received); // print all received transactions
			}
			try {
